import { EmbedBuilder, SlashCommandBuilder } from 'discord.js';
import { ChampionsModel } from '../models/champions.js';
import { getRandomItem, getNextChamp, handleErrors } from '../utilities.js';

export const data = new SlashCommandBuilder()
  .setName('habilidadv1')
  .setDescription(
    'Adivina de que campeón es la siguiente habilidad (imagen).'
  );
export async function execute(interaction) {
  if (interaction.channel.name !== 'bot-akshan')
    return interaction.editReply(
      'Ve al canal bot-akshan para jugar, si no existe crea uno con /canal.'
    );

  const champ = getNextChamp();
  console.log(champ);
  const abilities = await ChampionsModel.getAbilitiesIconsUrl({champ})

  const abilityEmbed = new EmbedBuilder()
    .setTitle('¿A qué campeón pertenece la siguiente habilidad?')
    .setImage(getRandomItem(abilities));

  interaction.editReply({ embeds: [abilityEmbed] }).catch((error) => {
    console.log(`Couldn't edit reply abilityV1 embed`, error);
    handleErrors(interaction, error);
    interaction.channel
      .send({ embeds: [abilityEmbed] })
      .catch((error2) => handleErrors(interaction, error2));
  });

  return;
}

/*

⡴⠑⡄⠀⠀⠀⠀⠀⠀⠀ ⣀⣀⣤⣤⣤⣀⡀
⠸⡇⠀⠿⡀⠀⠀⠀⣀⡴⢿⣿⣿⣿⣿⣿⣿⣿⣷⣦⡀
⠀⠀⠀⠀⠑⢄⣠⠾⠁⣀⣄⡈⠙⣿⣿⣿⣿⣿⣿⣿⣿⣆
⠀⠀⠀⠀⢀⡀⠁⠀⠀⠈⠙⠛⠂⠈⣿⣿⣿⣿⣿⠿⡿⢿⣆
⠀⠀⠀⢀⡾⣁⣀⠀⠴: ⣗⡀⠀⢻⣿⣿⠭⢤⣴⣦⣤⣹⠀⠀⠀⢀⢴⣶⣆
⠀⠀⢀⣾⣿⣿⣿⣷⣮⣽⣾⣿⣥⣴⣿⣿⡿⢂⠔⢚⡿⢿⣿⣦⣴⣾⠸⣼⡿
⠀⢀⡞⠁⠙⠻⠿⠟⠉⠀⠛⢹⣿⣿⣿⣿⣿⣌ ⣼⣿⣾⣿⡟⠉
⠀⣾⣷⣶⠇⠀⠀⣤⣄⣀⡀⠈⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡇
⠀⠉⠈⠉⠀⠀⢦⡈⢻⣿⣿⣿⣶⣶⣶⣶⣤⣽⡹⣿⣿⣿⣿⡇
⠀⠀⠀⠀⠀⠀⠀⠉⠲⣽⡻⢿⣿⣿⣿⣿⣿⣿⣷⣜⣿⣿⣿⡇
⠀⠀ ⠀⠀⠀⠀⠀⢸⣿⣿⣷⣶⣮⣭⣽⣿⣿⣿⣿⣿⣿⣿⠇
⠀⠀⠀⠀⠀⠀⣀⣀⣈⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠇
⠀⠀⠀⠀⠀⠀⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠃
*/
