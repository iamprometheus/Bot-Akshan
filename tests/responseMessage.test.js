import { arraysShareItem } from "../controllers/responseMessage.js";
...
test("Returns true for two arrays with the same elements", () => {
  const actual = arraysShareItem(["Superior", "Central"], ["Central", "Superior"])
  const expected = true
  assert.equal(actual, expected)
});

test("Returns 'partial' for two arrays with atleast one common element", () => {
  const actual = arraysShareItem(["Superior", "Central", "Jungla"], ["Central", "Superior"])
  const expected = 'partial'
  assert.equal(actual, expected)
});
...
test("Returns false if none of the arrays does not share items", () => {
  const actual = arraysShareItem(["Inferior", "Soporte"], ["Central", "Superior"])
  const expected = false
  assert.equal(actual, expected)
});
